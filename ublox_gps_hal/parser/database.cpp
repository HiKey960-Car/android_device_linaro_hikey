...
*/
/*******************************************************************************
 * $Id: database.cpp 64657 2013-01-10 11:00:39Z andrea.foni $
 ******************************************************************************/
#include "stdafx.h"
...
__drv_floatUsed double CDatabase::CalcLeapSeconds(double dUtcSec) const
{
	const struct 
	{
		double dTime; 
...
void CDatabase::CompleteHeading(void)
{
	...
	bool bMh  = varN[DATA_MAGNETIC_HEADING_DEGREES].Get(mh);
	bool bTh  = varN[DATA_TRUE_HEADING_DEGREES].Get(th);
...
void CDatabase::AddMessage(unsigned char* pData, int iSize)
{
	// Do nothing
	((void) (pData));
	((void) (iSize));
